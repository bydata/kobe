...
shots_train <- pp[["train"]]
shots_validation <- pp[["validation"]]
shots_test <- pp[["test"]]
...
xgb1 <- train(shot_made_flag ~ ., data = shots_train, method = "xgbTree", 
              metric = "logLoss", tuneLength = 12, trControl = control)
toc()
xgb1
...
pred_xgb1 <- predict(xgb1, newdata = shots_validation, type = "prob")
summary(pred_xgb1)
